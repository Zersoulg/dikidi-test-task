...
INSERT INTO motorbikes_types (name) VALUES ('pitbike');
...
-- fetch 
-- SELECT mt.name, COUNT(motorbikes_types_id) AS num_bikes
--     FROM motorbikes_types mt
-- LEFT JOIN motorbikes m ON m.motorbikes_types_id = mt.id
...
SELECT
  ...
  COUNT(m.id)
FROM motorbikes_types mt
LEFT JOIN motorbikes m
  ON m.motorbikes_types_id = mt.id
